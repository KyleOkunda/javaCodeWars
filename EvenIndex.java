...
the index ({1,2,3}) and the sum of the right side of 
the index ({3,2,1}) both equal 6.

Let's look at another one.
You are given the array {1,100,50,-51,1,1}:
Your function will return the index 1, because at the 
1st position of the array, the sum of left side of the
 index ({1}) and the sum of the right side of the index
...
Index 0 is the place where the left side and right side
 are equal.
...
The lowest index N where the side to the left of N is
 equal to the side to the right of N. If you do not 
 find an index that fits these rules, then you will 
 return -1.
...
public class EvenIndex {
     ...
     static int findEvenIndex(int[] arr){
        ...
        for(int i = 0; i < arr.length; i++){
            int sumRightSide = 0, sumLeftSide = 0;
            ...
            if(i == 0 && sumRightSide == 0){
                System.out.println("First index: " + i);
                return i;
            }
            
            for(int reversal = i; reversal >= 0; reversal--){
                
                if(reversal == i){
                    continue;
                }
                sumLeftSide += arr[reversal];
    ...
    public static void main(String[] args) {
        int[] myArr = {20,10,-80,10,10,15,35};
        findEvenIndex(myArr);
    }
